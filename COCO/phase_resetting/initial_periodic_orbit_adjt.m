...
% Add adjoint?
prob = adjt_isol2coll(prob, 'initial_PO');
...
% Read index data for adjoint segment
[data_adjt, aidx] = coco_get_adjt_data(prob, 'initial_PO.coll', 'data', 'axidx');
...
opt = data_adjt.coll_opt;

%-----------------------------%
%     Boundary Conditions     %
%-----------------------------%
% Apply periodic orbit boundary conditions and special phase condition
prob = coco_add_func(prob, 'bcs_po', bcs_funcs.bcs_PO{:}, data, 'zero', 'uidx', ...
                   uidx([maps.x0_idx; ...
                         maps.x1_idx; ...
                         maps.p_idx]));
...
prob = coco_add_adjt(prob, 'bcs_PO', 'aidx', ...
                     aidx([opt.x0_idx; opt.x1_idx; opt.p_idx]));
...
%------------------%
% Run COCO continuation
coco(prob, run_new, [], 1, {'A', 'gamma', 'd.A', 'd.gamma'}, A_range);
