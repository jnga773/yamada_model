...
% Print to console
fprintf("~~~ Phase Reset: Second Run (phase_reset_2_theta_perturb_multi.m) ~~~ \n");
fprintf('Fix A_perturb, continue in theta_perturb \n');
fprintf('Run name: %s \n', run_new);
...
function phase_transition_scan_A_perturb(run_new_in, run_old_in, label_old_in, data_PR_in, bcs_funcs_in)
  ...
  SP_values = 0 : 0.01*pi : 0.5*pi;
  
  % When the parameter we want (from param) equals a value in A_vec
  prob = coco_add_event(prob, 'SP', 'theta_perturb', SP_values);

  % Run COCO continuation
  prange = {[0.0, 0.5*pi], [], [-1e-4, 1e-2], [0.99, 1.01], [0.99, 1.1]};
  coco(prob, run_new_in, [], 1, {'theta_perturb', 'theta_new', 'eta', 'mu_s', 'A_perturb', 'T'}, prange);
