function plot_orbit_and_W_PO_solution(run_in, label_in)
  % plot_orbit_and_W_PO_solution(run_in, label_in)
  %
  ...
  plot3(ax, x1(:, 1), x1(:, 2), x1(:, 3), Color=colours(1, :), ...
        DisplayName='$W^{ss}_{1}(\Gamma)$');

  % Plot W2
  plot3(ax, x2(:, 1), x2(:, 2), x2(:, 3), Color=colours(2, :), ...
        DisplayName='$W^{ss}_{2}(\Gamma)$');
