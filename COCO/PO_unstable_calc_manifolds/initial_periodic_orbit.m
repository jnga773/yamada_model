...
prob = coco_add_event(prob, 'H_PT', 'A', 6.85);
...
prob = coco_add_event(prob, 'PO_PT', 'gamma', 6.53e-2);
...
%----------------------------%
% Calculate dem tings
data_unstable = calc_PO_initial_solution(run_old, label_old(1));
data_stable = calc_PO_initial_solution(run_old, label_old(2));
...
prob = ode_isol2coll(prob, 'PO_unstable', funcs.field{:}, ...
                     data_unstable.t, data_unstable.x, data_unstable.p, ...
                     '-var', eye(3));
...
% label_plot = coco_bd_labs(coco_bd_read(run_new), 'PO_PT');
% 
% [sol, ~] = coll_read_solution('PO_unstable', run_new, label_plot);
% xbp_PO = sol.xbp;
% fprintf('Head-point = (%.3f, %.3f, %.3f)\n', xbp_PO(1, :));
% 
% % Monodromy matrix
% chart = coco_read_solution('PO_unstable.coll.var', run_new, label_plot, 'chart');
% data  = coco_read_solution('PO_unstable.coll', run_new, label_plot, 'data');
% 
