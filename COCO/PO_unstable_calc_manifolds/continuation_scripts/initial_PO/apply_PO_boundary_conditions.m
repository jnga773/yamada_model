function prob_out = apply_PO_boundary_conditions(prob_in, bcs_PO_in)
  ...
  prob = coco_add_glue(prob, 'pars_glue', ...
                       [uidx_s(maps_s.p_idx); uidx_s(maps_s.p_idx); uidx_s(maps_s.p_idx); uidx_s(maps_s.p_idx)], ...
                       [uidx_u(maps_u.p_idx); uidx_0(maps_0.p_idx); uidx_pos(maps_pos.p_idx); uidx_neg(maps_neg.p_idx)]);

  %-----------------------------%
  %     Boundary Conditions     %
  %-----------------------------%
  % Apply periodic orbit boundary conditions and special phase condition
  prob = coco_add_func(prob, 'bcs_PO_stable', bcs_PO_in{:}, data_s, 'zero', 'uidx', ...
                       uidx_s([maps_s.x0_idx(1:data_s.xdim); ...
                               maps_s.x1_idx(1:data_s.xdim); ...
                               maps_s.p_idx(1:data_s.pdim)]));
  prob = coco_add_func(prob, 'bcs_PO_unstable', bcs_PO_in{:}, data_u, 'zero', 'uidx', ...
