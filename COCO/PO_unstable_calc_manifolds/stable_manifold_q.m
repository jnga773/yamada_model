...
% Print to console
fprintf("~~~ Stable Manifold (q): First Run ~~~ \n");
...
prob = ode_isol2coll(prob, 'PO_unstable', funcs.field{:}, ...
                     data_isol.tbp_u, data_isol.xbp_u, data_isol.p);
...
fprintf('Close the epsilon distance \n');
fprintf('Run name: %s \n', run_new);
fprintf('Continuing from point %d in run: %s \n', label_old, run_old);
...
% Set step size
prob = coco_set(prob, 'cont', 'h_min', 1e-3, 'h', 1e-2, 'h_max', 5e-2);
...
%     Add CoCo Event     %
%------------------------%
prob = coco_add_event(prob, 'EPS0', 'eps', 1e-10);
