function plot_hopf_to_PO_solution(run_in, label_in)
  ...
  % Sort labels
  label_in = sort(label_in);
  
  %-----------------------------------%
  %     Read Data: Unstable Orbit     %
  %-----------------------------------%
  % Read da solution g
  sol_u = po_read_solution('', run_in, label_in(1));
  xbp_u = sol_u.xbp;
  ...
  %-----------------------------------%
  % Read da solution g
  sol_s = po_read_solution('', run_in, label_in(2));
  xbp_s = sol_s.xbp;
  ...
  sol_0 = ep_read_solution('x0', run_in, label_in(2));
  sol_pos = ep_read_solution('xpos', run_in, label_in(2));
  sol_neg = ep_read_solution('xneg', run_in, label_in(2));
  ...
  plot3(ax, xbp_u(:, 1), xbp_u(:, 2), xbp_u(:, 3), LineStyle='-', Color=colours(4, :), ...
        DisplayName='Unstable Orbit');
